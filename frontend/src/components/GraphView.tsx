  ...
  return (
    <div ref={containerRef} className="flex-1 relative bg-gray-900">
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
        ...
        {/* Third row: edge type filters */}
        <div className="bg-gray-800 rounded-lg shadow-lg flex items-center px-2 py-1 gap-1">
          <span className="text-xs text-gray-500 mr-1">Edges:</span>
          {EDGE_TYPES.map((edgeType) => (
            <button
              key={edgeType}
              onClick={() => onEdgeTypeToggle(edgeType)}
              className={`px-2 py-0.5 text-xs rounded flex items-center gap-1 transition-colors ${
                filters.edgeTypes.has(edgeType)
                  ? "bg-gray-600 text-gray-100"
                  : "bg-gray-700/50 text-gray-500"
              }`}
              title={`Toggle ${edgeType} edges`}
            >
              <span
                className="w-2 h-0.5 inline-block rounded"
                style={{ backgroundColor: EDGE_TYPE_COLORS[edgeType as keyof EdgeTypeFilters] }}
              />
              {edgeType}
            </button>
          ))}
